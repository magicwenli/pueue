...
#[tokio::test]
async fn test_tcp_socket() -> Result<()> {
    better_panic::install();
    let (mut shared_settings, _tempdir) = helper::get_shared_settings(false);

    shared_settings.insane_allow_insecure_connections = Some(true);
